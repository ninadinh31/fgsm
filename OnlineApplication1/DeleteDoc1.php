...
$colname_rsDocs = "-1";
if (isset($_GET['docID'])) {
  $colname_rsDocs = $_GET['docID'];
}
mysql_select_db($database_FGSP, $FGSP);
$query_rsDocs = sprintf("SELECT * FROM tblsqdocuments WHERE docID = %s", GetSQLValueString($colname_rsDocs, "int"));
$rsDocs = mysql_query($query_rsDocs, $FGSP) or die(mysql_error());
...
if ((isset($_GET['docID'])) && ($_GET['docID'] != "")) {
  ...
  unlink('uploads/'.$row_rsDocs['docName']);
